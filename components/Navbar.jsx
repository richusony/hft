            ...
            <>
                <div keys={keys} className="transition duration-[1000ms] flex justify-between ease-linear p-3 text-center">
                    ...
                    <ul className={`transition duration-[1000ms] rounded p-3 md:p-0 md:bg-inherit ease-in bg-[#efeded] absolute z-10 md:static ${cname} md:opacity-100 md:translate-x-0 top-14 right-0 md:flex justify-between`}>
                        ...
                        <li className="transition duration-[1000ms] ease-linear">{localStorage.getItem('uname') ? <div className="transition duration-[1000ms] my-auto ease-in rounded-[55%]"><FontAwesomeIcon onClick={() => { dropdown ? setDropdown(false) : setDropdown(true) }} className="transition duration-[1000ms] text-2xl mt-3 md:p-0 md:mx-auto border-b-2 border-black md:border-none cursor-pointer select-none ease-linear hover:font-medium hover:italic" icon={faUser} /></div> : 
                        <h3 id='new' className="transition duration-150 text-xl px-3 py-2 md:p-0 md:mx-auto w-[100px] md:border-none select-none md:rounded md:bg-[#151522] md:text-white cursor-pointer ease-in hover:scale-95"><Link href="/Login">Login</Link></h3>}
